...
import base64  # 导入base64模块，用于对前端传来的音频数据进行解码
import tempfile  # 导入临时文件模块，用于将base64音频临时落盘以便SDK读取


def transcribe(file_urls=None, audio_base64=None):  # 定义转写函数，支持两种输入：公网URL列表或Base64音频数据
    """
    使用达摩盘(DashScope) ASR服务进行转写

    支持两种输入：
    1) file_urls: 语音文件的公网URL列表，例如 ["https://.../a.wav"]
    2) audio_base64: 前端提交的音频数据（base64字符串）。将临时落盘后尝试调用SDK。
    ...
    try:  # 使用try捕获整个转写流程中的异常，保证函数返回统一结构
        ...
        # 其次尝试处理base64音频
        if audio_base64 and isinstance(audio_base64, str) and len(audio_base64) > 0:  # 检查audio_base64是否为非空字符串
            # 兼容可能带有data:audio/...;base64, 前缀的情况
            comma_idx = audio_base64.find(',')  # 查找逗号位置，用于判断是否存在data URL前缀（如"data:audio/wav;base64,xxx"）
            if comma_idx != -1 and audio_base64[:comma_idx].lower().startswith('data:'):  # 若存在前缀且以data:开头
                audio_b64 = audio_base64[comma_idx + 1:]  # 切掉前缀，只保留逗号后面的base64主体
            else:
                audio_b64 = audio_base64  # 否则直接使用传入的字符串作为base64主体

            audio_bytes = base64.b64decode(audio_b64)  # 将base64字符串解码为二进制音频数据
            # 临时写入文件
            with tempfile.NamedTemporaryFile(delete=False, suffix='.wav') as tmpf:  # 创建一个不自动删除的临时文件，后缀为.wav
                tmpf.write(audio_bytes)  # 将解码后的音频字节写入临时文件
                tmp_path = tmpf.name  # 记录临时文件路径，供后续SDK调用使用

            # 注意：DashScope SDK常见用法是传入file_urls。如果支持本地文件，需要查阅SDK文档。
            # 这里尝试使用可能存在的file_paths参数；若SDK不支持，将抛出异常。
            try:  # 尝试以本地文件路径方式调用SDK
                task_response = Transcription.async_call(  # 提交异步任务
                    model="fun-asr-mtl",  # 指定ASR模型
                    file_paths=[tmp_path]  # 若SDK支持本地文件，将传入临时文件路径列表；不支持时这里会抛异常
                )
            ...
            except Exception as e:  # 捕获调用本地文件方式可能出现的异常（比如参数不被支持）
                return {
                    "success": False,  # 失败标记
                    "error": "SDK当前可能不支持直接传本地文件，请使用file_urls方式",  # 提示使用URL方式
                    "details": str(e)  # 返回具体异常信息便于定位问题
                }
            finally:
                try:
                    if 'tmp_path' in locals() and os.path.exists(tmp_path):  # 若临时文件存在，则进行清理
                        os.remove(tmp_path)  # 删除临时文件，避免磁盘空间泄露
                except Exception:
                    pass  # 清理失败时忽略异常，防止影响主流程

        # 两种输入均未提供
        return {
            "success": False,  # 失败标记
            "error": "必须提供file_urls或audio_base64其中之一"  # 参数校验失败的提示信息
        }
